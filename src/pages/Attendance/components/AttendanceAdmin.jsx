...
export default function AttendanceAdmin() {
  ...
  const getAllStudents = (secteurData, absentData) => {
    const allStudents = []

    secteurData.forEach((secteur) => {
  ...
  const handleInputChange = (studentId, field, value, month = null) => {
    ...
    const updatedStudents = filteredStudents.map((student) => {
      if (student.cef === studentId) {
        ...
        if (field === "anj") {
          if (month) {
            updatedStudent.anj = { ...updatedStudent.anj, [month]: Number.parseInt(value, 10) || 0 }
          } else {
            updatedStudent.totalANJ = Number.parseInt(value, 10) || 0
          }
        } else if (field === "retards" || field === "aj") {
          updatedStudent[field] = Number.parseInt(value, 10) || 0
        }
  ...
  const renderAdminTableCells = (student) => (
    <>
      ...
      <td className="px-4 py-2">{disciplineNotes[student.cef] || student.disciplineNote || 15}/15</td>
    </>
  )
